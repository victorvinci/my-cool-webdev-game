...
A playground for building simple browser games. Forked from [`steamdeck-webdev-template`](https://github.com/victorvinci/steamdeck-webdev-template) — an Nx monorepo with a React + Vite + TanStack Router frontend, an Express backend, and MySQL.

Games are built client-side on an HTML5 `<canvas>` inside `apps/frontend`. Run `npm run fe` to start the dev server.

> The sections below are inherited from the template and document the full backend/DB/CI tooling. Much of it (MySQL, the Express API, release ceremony) is optional for a client-side game — treat it as available, not required.
